...
export function logStartupPath() {
  ...
  const hasDedicatedGraph = Boolean(process.env.GRAPH_LLM_API_KEY?.trim() && process.env.GRAPH_LLM_MODEL?.trim());
  const graphLlmKey = process.env.GRAPH_LLM_API_KEY?.trim() || llmApiKey;
  const graphLlmModel = process.env.GRAPH_LLM_MODEL?.trim() || llmModel;
  const graphLlmUrl = process.env.GRAPH_LLM_BASE_URL?.trim() || llmBaseUrl;
  const databaseUrl = process.env.DATABASE_URL?.trim();

  console.log('[startup] ===== 知研 启动路径 =====');
  console.log(`[startup] 知乎检索: ${zhihuSecret ? '✅ CONFIGURED' : '❌ EMPTY_OR_MISSING (将使用Demo数据)'}`);
  console.log(`[startup] 知乎API地址: ${zhihuBaseUrl || '默认 (developer.zhihu.com)'}`);
  console.log(`[startup] LLM API: ${(llmApiKey && llmModel) ? '✅ CONFIGURED' : '❌ EMPTY_OR_MISSING (将使用Fixture生成器)'}`);
  ...
  console.log(`[startup] 图谱 LLM: ${(graphLlmKey && graphLlmModel) ? (hasDedicatedGraph ? '✅ 已配置独立端点 (model=' + graphLlmModel + ')' : '✅ 已复用通用LLM配置 (model=' + graphLlmModel + ')') : '❌ 未配置'}`);
