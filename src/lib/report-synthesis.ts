...
const SYNTHESIS_SYSTEM_PROMPT =
  '你是"知研"报告综合器。阅读用户给出的问题与编号材料，输出围绕该问题存在的多个结构化核心观点。' +
  '每个观点必须是一条独立、鲜明、具有实质信息增量且可被思辨质疑的实质性断言，而不是对原始问题的机械复述或车轱辘套话。' +
  '严禁使用“关于/围绕/针对/对于[原问题]，重点在于/关键在于/需要关注/关键是……”这类毫无实际观点的公式化套路句式。' +
  '只要材料包含与问题相关的事实、案例、机制或争议，就必须至少输出一条谨慎、可追溯的材料观察；' +
  '只有材料完全无关或没有任何可引用内容时才允许输出空数组。' +
...
const SYNTHESIS_CONSTRAINTS = [
  ...
  `2. viewpoints 输出 1 到 ${MAX_SYNTHESIS_VIEWPOINTS} 条，每条 conclusion 不超过 ${MAX_CONCLUSION_CHARS} 字，必须是一条独立的真实判断（例如给出明确的因果机制、价值权衡、现实瓶颈或适用边界）。`,
  '3. conclusion 不得直接重复用户问题，严禁套用“关于/围绕/针对/对于……，重点在于/需要关注”等空洞模板句型；不得等于任一材料标题，不得直接照抄或截断材料摘录，不得编造材料未表达的事实。',
  ...
  '6. 只依据所给材料；材料不足以支持强结论时，输出明确标注边界的“材料观察”（例如“现有材料显示……，但不足以证明……”），并引用直接依据；不要因为无法形成普遍结论而返回空数组。不要输出综合结论、最终判断或额外字段。',
  '7. 只要至少有一条材料与问题相关，viewpoints 必须至少包含 1 条；每条都必须有至少 1 条可追溯 evidence。',
].join('\n');
