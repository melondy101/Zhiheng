...
6. **成果卡** → 6 段可追溯（初始表达/起始立场/新增证据/观点修正/你最后表达的观点/未解决问题）；会话完成时额外生成一次 AI 总结（初始观点/最终观点各一句话，标注"AI 总结"并附来源消息引用），与用户原话区块并列展示，不冒充用户原话；LLM 未配置或校验失败时该区块不显示，不影响 6 段可追溯记录
